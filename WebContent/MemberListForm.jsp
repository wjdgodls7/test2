...
<body>
	...
	<!-- ======= Hero Section ======= -->
	<section id="hero">
		<div class="hero-container" style="padding: 0 400px;">
			<h1>ICIA D CLASS MANAGEMENT</h1>
			<br/><br/>
			<section>
...
<%-- 									<td>${list.userTeam}</td>
									...
									<c:if test="${Level eq 'M' }">
										<td>
										<input type="button" onclick="location.href='memberView?id=${list.userId}'" value="정보 보기" style="width: 170px; height: 42px;"></td>
									</c:if>
									<c:if test="${Level eq 'M' }">
										<td><input type="button" onclick="confirm_test()" value="회원 추방" style="width: 170px; height: 42px;"></td>
									</c:if>
...
function confirm_test() {
	var result = confirm("정말로 추방 시키겠습니까?");
    if ( result ) {
    	location.href = "Memberfire?id=${id}&nick=${nick}";
    } else  {
