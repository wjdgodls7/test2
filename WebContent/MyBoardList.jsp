	...
	<section id="hero">
		<div class="hero-container" style="padding: 0 50px;">
					...
					<tbody>
						<c:forEach items="${boardList }" var="list">
							<tr>
								<td>${list.subject }</td>
								<td>${list.num}</td>
								<td><a href="writeDetail?num=${list.num }" style="color:black;">${list.title }</a></td>
								<td>${list.nick}</td>
								<td>${list.date}</td>
								<td>${list.hit}</td>
								<td><a href="BoardMyDeleteController?num=${list.num}" style="color:black;">삭제</a></td>
							</tr>
						</c:forEach>
						<!-- 				<tr class='even'>
			...
			<c:if test="${paging.page>1}">
				<a href="myWriteList?page=${paging.page-1}">[이전]</a>&nbsp; &nbsp; &nbsp;
	</c:if>
			...
			&nbsp; &nbsp; &nbsp;<a href="myWriteList?page=${paging.page+1}">[다음]</a>
			</c:if>
			</th>
		</tr>
	</table>
